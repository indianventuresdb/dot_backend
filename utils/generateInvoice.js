...
function generateInvoice(orderID, address, products, outputPath) {
  ...
  doc.fontSize(20);
  // Title
  doc.font("Helvetica-Bold").fontSize(28).text("Invoice", { align: "center" });
  doc.moveDown();
  ...
  products.forEach((item, index) => {
    ...
    doc.rect(headerX, rowY, cellWidth, cellHeight + 20 * (height - 1)).stroke();
    ...
    doc
      ...
      .stroke();
    doc.text(`$${item.price}`, headerX + 2 * cellWidth + 10, rowY + 10);
    ...
    doc
      ...
      .stroke();
    doc.text(`$${item.price}`, headerX + 3 * cellWidth + 10, rowY + 10);
    ...
    doc
      ...
      .stroke();
    doc.text(`$${item.price}`, headerX + 4 * cellWidth + 10, rowY + 10);
  ...
  const totalAmount = 5463;
  doc.text(`Sub Total: $${totalAmount.toFixed(2)}`);
  doc.text(`Courier Charge: $${totalAmount.toFixed(2)}`);
  doc.text(`GST: $${totalAmount.toFixed(2)}`);
